...
function Container.new( msg )
    local self = {parent=nil, message=msg, children={}}
...
function Container.is_reply( self )
    ...
    return string.match(subject, "R[eE]:%s?")
        or string.match(subject, "R[eE]%[%d%]:%s?")
end

--
-- Recursively delete all empty containers under this.
...
function Threader.thread( messages )
    ...
    for _,msg in ipairs(messages) do
        ...
        for _,ref in ipairs(par:get_references()) do
            ...
            -- loop.
            if (prev and not cur.parent
                and not cur:has_descandent(prev)) then
                     prev:add_child(cur)
            end
            prev = cur
        end
