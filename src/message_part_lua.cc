...
CMessagePart *
l_CheckCMessagePart (lua_State * l, int n)
{
    return *(CMessagePart **) luaL_checkudata (l, n, "luaL_CMessagePart");
}

int
l_CMessagePart_content (lua_State * l)
{
...
int
l_CMessagePart_filename (lua_State * l)
{
    CMessagePart *foo = l_CheckCMessagePart (l, 1);
...
int
l_CMessagePart_is_attachment (lua_State * l)
{
    CMessagePart *foo = l_CheckCMessagePart (l, 1);
...
int
l_CMessagePart_type (lua_State * l)
{
    CMessagePart *foo = l_CheckCMessagePart (l, 1);
...
int
l_CMessagePart_destructor (lua_State * l)
{
    CMessagePart *foo = l_CheckCMessagePart (l, 1);
    delete foo;
    return 0;
